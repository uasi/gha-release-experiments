...
fn load_credentials() -> Result<Credentials> {
    ...
    let (consumer_key, consumer_secret) = match (CONSUMER_KEY, CONSUMER_SECRET) {
        ...
        _ => bail!("Could not find login information. Try `phog login --with-credentials`."),
    };
    ...
    match load_access_token() {
        ...
        Err(e) => Err(e).context("Could not find login information. Try `phog login`.")?,
    }
}
